...
namespace CNGE {
	...
	auto Window::windowProc(HWND const window, UINT const message, WPARAM const wParam, LPARAM const lParam) -> LONG WINAPI {
		...
		case WM_LBUTTONUP: {
			input.keys[Input::BUTTON_LEFT] = Input::BUTTON_RELEASED;
		} return 0;

		case WM_MBUTTONUP: {
			input.keys[Input::BUTTON_MIDDLE] = Input::BUTTON_RELEASED;
		} return 0;
			
		case WM_RBUTTONUP: {
			input.keys[Input::BUTTON_RIGHT] = Input::BUTTON_RELEASED;
		} return 0;
