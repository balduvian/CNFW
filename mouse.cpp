...
HWND
CreateOpenGLWindow(const wchar_t* title, int x, int y, int width, int height,
	...
	LPSTR lpszCmdLine, int nCmdShow)
{
	...
	hWnd = CreateOpenGLWindow(L"mouse", 0, 0, 256, 256, color, buffer);
	if (hWnd == NULL)
		exit(1);
	...
	auto* const icon = LoadIcon(hCurrentInst, MAKEINTRESOURCE(143));
	SendMessageA(hWnd, WM_SETICON, 1, LPARAM(icon));
